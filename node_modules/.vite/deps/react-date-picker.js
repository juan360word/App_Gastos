...
function getFormatter$1(options) {
	return function formatter(locale, date) {
		...
		if (!formatterCache$1.has(localeWithDefault)) formatterCache$1.set(localeWithDefault, /* @__PURE__ */ new Map());
		const formatterCacheLocale = formatterCache$1.get(localeWithDefault);
...
function getSafeFormatter$1(options) {
	return (locale, date) => getFormatter$1(options)(locale, toSafeHour$1(date));
}
...
getSafeFormatter$1(formatDateOptions);
...
function getISOLocalMonth(date) {
	return `${padStart(getYear(date), 4)}-${padStart(getMonthHuman(date))}`;
}
/**
...
function getISOLocalDate(date) {
	return `${padStart(getYear(date), 4)}-${padStart(getMonthHuman(date))}-${padStart(getDate(date))}`;
}
//#endregion
...
function Month({ classes = [], formatMonth = formatMonth$1, formatMonthYear: formatMonthYear$1 = formatMonthYear, ...otherProps }) {
	const { date, locale } = otherProps;
	return (0, import_jsx_runtime.jsx)(Tile, {
		...otherProps,
...
function getValueType$1(view) {
	return allValueTypes$1[allViews$1.indexOf(view)];
}
function getValue$1(value, index) {
	const rawValue = Array.isArray(value) ? value[index] : value;
	if (!rawValue) return null;
	const valueDate = toDate$1(rawValue);
	if (Number.isNaN(valueDate.getTime())) throw new Error(`Invalid date: ${value}`);
	return valueDate;
}
function getDetailValue$1({ value, minDate, maxDate, maxDetail }, index) {
	const valuePiece = getValue$1(value, index);
	if (!valuePiece) return null;
	const valueType = getValueType$1(maxDetail);
	return between$1((() => {
		switch (index) {
			case 0: return getBegin$1(valueType, valuePiece);
			case 1: return getEnd$1(valueType, valuePiece);
			default: throw new Error(`Invalid index value: ${index}`);
		}
	})(), minDate, maxDate);
}
var getDetailValueFrom$1 = (args) => getDetailValue$1(args, 0);
var getDetailValueTo$1 = (args) => getDetailValue$1(args, 1);
var getDetailValueArray$1 = (args) => [getDetailValueFrom$1, getDetailValueTo$1].map((fn) => fn(args));
function getActiveStartDate({ maxDate, maxDetail, minDate, minDetail, value, view }) {
	return getBegin$1(getView(view, minDetail, maxDetail), getDetailValueFrom$1({
		value,
		minDate,
		maxDate,
		maxDetail
...
function getInitialActiveStartDate({ activeStartDate, defaultActiveStartDate, defaultValue, defaultView, maxDate, maxDetail, minDate, minDetail, value, view }) {
	...
	if (valueFrom) return getBegin$1(rangeType, valueFrom);
...
var dist_default$2 = (0, import_react.forwardRef)(function Calendar(props, ref) {
	const { activeStartDate: activeStartDateProps, allowPartialRange, calendarType, className, "data-testid": dataTestId, defaultActiveStartDate, defaultValue, defaultView, formatDay, formatLongDate, formatMonth, formatMonthYear, formatShortWeekday, formatWeekday, formatYear, goToRangeStartOnSelect = true, inputRef, locale, maxDate = defaultMaxDate$1, maxDetail = "month", minDate = defaultMinDate$1, minDetail = "century", navigationAriaLabel, navigationAriaLive, navigationLabel, next2AriaLabel, next2Label, nextAriaLabel, nextLabel, onActiveStartDateChange, onChange: onChangeProps, onClickDay, onClickDecade, onClickMonth, onClickWeekNumber, onClickYear, onDrillDown, onDrillUp, onViewChange, prev2AriaLabel, prev2Label, prevAriaLabel, prevLabel, returnValue = "start", selectRange, showDoubleView, showFixedNumberOfWeeks, showNavigation = true, showNeighboringCentury, showNeighboringDecade, showNeighboringMonth = true, showWeekNumbers, tileClassName, tileContent, tileDisabled, value: valueProps, view: viewProps } = props;
	...
	const [valueState, setValueState] = (0, import_react.useState)(Array.isArray(defaultValue) ? defaultValue.map((el) => el !== null ? toDate$1(el) : null) : defaultValue !== null && defaultValue !== void 0 ? toDate$1(defaultValue) : null);
	...
	const value = (() => {
		...
		if (!rawValue) return null;
		return Array.isArray(rawValue) ? rawValue.map((el) => el !== null ? toDate$1(el) : null) : rawValue !== null ? toDate$1(rawValue) : null;
	})();
	const valueType = getValueType$1(maxDetail);
	...
	const getProcessedValue = (0, import_react.useCallback)((value) => {
		return (() => {
			switch (returnValue) {
				case "start": return getDetailValueFrom$1;
				case "end": return getDetailValueTo$1;
				case "range": return getDetailValueArray$1;
	...
	const drillUp = (0, import_react.useCallback)(() => {
		...
		const nextActiveStartDate = getBegin$1(nextView, activeStartDate);
		setActiveStartDateState(nextActiveStartDate);
		setViewState(nextView);
		const args = {
			action: "drillUp",
	...
	const onChange = (0, import_react.useCallback)((rawNextValue, event) => {
		...
		let nextValue;
		if (selectRange) if (isFirstValueInRange) nextValue = getBegin$1(valueType, rawNextValue);
		else {
	...
	function renderContent(next) {
		const commonProps = {
			activeStartDate: next ? getBeginNext(view, activeStartDate) : getBegin$1(view, activeStartDate),
			hover,
			locale,
			maxDate,
			minDate,
	...
	return (0, import_jsx_runtime.jsxs)("div", {
		className: clsx(baseClassName$1, selectRange && (Array.isArray(value) ? value : [value]).length === 1 && `${baseClassName$1}--selectRange`, showDoubleView && `${baseClassName$1}--doubleView`, className),
		"data-testid": dataTestId,
		ref: inputRef,
		children: [renderNavigation(), (0, import_jsx_runtime.jsxs)("div", {
			className: `${baseClassName$1}__viewContainer`,
...
//#endregion
//#region node_modules/detect-element-overflow/dist/index.js
...
//#endregion
//#region node_modules/react-fit/dist/Fit.js
...
function Fit({ children, invertAxis, invertSecondaryAxis, mainAxis = "y", spacing = 8 }) {
	...
	const fit = (0, import_react.useCallback)(() => {
		...
		if (!parent) return;
		const { position } = window.getComputedStyle(element.current);
		if (position !== "absolute") element.current.style.position = "absolute";
		const { position: parentPosition } = window.getComputedStyle(parent);
	...
	(0, import_react.useEffect)(() => {
		...
		if (isMutationObserverSupported && element.current) new MutationObserver(onMutation).observe(element.current, {
			attributes: true,
	...
	return (0, import_jsx_runtime.jsx)("span", {
		ref: (domContainer) => {
			...
			assignRefs(domContainer === null || domContainer === void 0 ? void 0 : domContainer.firstElementChild);
		},
...
//#endregion
//#region node_modules/react-fit/dist/index.js
var dist_default$1 = Fit;
//#endregion
//#region node_modules/update-input-width/dist/esm/index.js
...
function measureText(text, font) {
	var context = (cachedCanvas || (cachedCanvas = document.createElement("canvas"))).getContext("2d");
...
function updateInputWidth(element) {
	...
	var width = measureText(element.value || element.placeholder, font);
	if (width === null) return null;
	element.style.width = "".concat(width, "px");
	return width;
}
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/Input.js
...
function onFocus(event) {
	const { target } = event;
	if (isIEOrEdgeLegacy) requestAnimationFrame(() => target.select());
	else target.select();
}
...
function Input({ ariaLabel, autoFocus, className, disabled, inputRef, max, min, name, nameForClass, onChange, onKeyDown, onKeyUp, placeholder = "--", required, showLeadingZeros, step, value }) {
	...
	return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [hasLeadingZero ? (0, import_jsx_runtime.jsx)("span", {
...
//#endregion
//#region node_modules/react-date-picker/dist/shared/utils.js
/**
* Returns a value no smaller than min and no larger than max.
*
* @param {Date} value Value to return.
...
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/DayInput.js
function DayInput({ maxDate, minDate, month, year, ...otherProps }) {
	...
	return (0, import_jsx_runtime.jsx)(Input, {
		max: safeMin(currentMonthMaxDays, maxDate && isSameMonth(maxDate) && getDate(maxDate)),
		min: safeMax(1, minDate && isSameMonth(minDate) && getDate(minDate)),
		name: "day",
		...otherProps
	});
}
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/MonthInput.js
function MonthInput({ maxDate, minDate, year, ...otherProps }) {
	function isSameYear(date) {
		return date && year === getYear(date).toString();
	}
	return (0, import_jsx_runtime.jsx)(Input, {
		max: safeMin(12, maxDate && isSameYear(maxDate) && getMonthHuman(maxDate)),
		min: safeMax(1, minDate && isSameYear(minDate) && getMonthHuman(minDate)),
		name: "month",
		...otherProps
	});
}
//#endregion
//#region node_modules/react-date-picker/dist/shared/dateFormatter.js
...
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/MonthSelect.js
function MonthSelect({ ariaLabel, autoFocus, className, disabled, inputRef, locale, maxDate, minDate, onChange, onKeyDown, placeholder = "--", required, short, value, year }) {
	...
	return (0, import_jsx_runtime.jsxs)("select", {
		...
		required,
		value: value !== null ? value : "",
		children: [!value && (0, import_jsx_runtime.jsx)("option", {
			value: "",
...
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/NativeInput.js
function NativeInput({ ariaLabel, disabled, maxDate, minDate, name, onChange, required, value, valueType }) {
	...
	const nativeValueParser = (() => {
		switch (valueType) {
			...
			case "month": return getISOLocalMonth;
			case "day": return getISOLocalDate;
			default: throw new Error("Invalid valueType");
		}
	})();
	...
	return (0, import_jsx_runtime.jsx)("input", {
		...
		disabled,
		hidden: true,
		max: maxDate ? nativeValueParser(maxDate) : void 0,
		min: minDate ? nativeValueParser(minDate) : void 0,
		name,
		onChange,
...
//#endregion
//#region node_modules/react-date-picker/dist/DateInput/YearInput.js
function YearInput({ maxDate, minDate, placeholder = "----", valueType, ...otherProps }) {
	return (0, import_jsx_runtime.jsx)(Input, {
		max: safeMin(275760, maxDate && getYear(maxDate)),
		min: safeMax(1, minDate && getYear(minDate)),
		name: "year",
		placeholder,
		step: (() => {
...
//#endregion
//#region node_modules/react-date-picker/dist/Divider.js
...
//#endregion
//#region node_modules/react-date-picker/dist/shared/dates.js
/**
* Returns the beginning of a given range.
*
* @param {RangeType} rangeType Range type (e.g. 'day')
...
//#endregion
//#region node_modules/react-date-picker/dist/DateInput.js
...
function renderCustomInputs(placeholder, elementFunctions, allowMultipleInstances) {
	...
	return placeholder.split(pattern).reduce((arr, element, index) => {
		const divider = element && (0, import_jsx_runtime.jsx)(Divider, { children: element }, `separator_${index}`);
		arr.push(divider);
...
function DateInput({ autoFocus, className, dayAriaLabel, dayPlaceholder, disabled, format, isCalendarOpen: isCalendarOpenProps = null, locale, maxDate, maxDetail = "month", minDate, monthAriaLabel, monthPlaceholder, name = "date", nativeInputAriaLabel, onChange: onChangeProps, onInvalidChange, required, returnValue = "start", showLeadingZeros, value: valueProps, yearAriaLabel, yearPlaceholder }) {
	const [year, setYear] = (0, import_react.useState)(null);
	...
	const yearInput = (0, import_react.useRef)(null);
	const monthInput = (0, import_react.useRef)(null);
	const monthSelect = (0, import_react.useRef)(null);
	const dayInput = (0, import_react.useRef)(null);
	const [isCalendarOpen, setIsCalendarOpen] = (0, import_react.useState)(isCalendarOpenProps);
	const lastPressedKey = (0, import_react.useRef)(void 0);
	const hasPendingInternalChange = (0, import_react.useRef)(false);
	(0, import_react.useEffect)(() => {
	...
	const formatDate = (() => {
		...
		return getFormatter(getFormatterOptionsCache[level] || (() => {
			const options = { year: "numeric" };
			if (level >= 2) options.month = "numeric";
			if (level >= 3) options.day = "numeric";
	...
	*/
	...
	const placeholder = format || (() => {
		...
		function formatDatePiece(name, dateToFormat) {
			return getFormatter(getFormatterOptionsCache[name] || (() => {
	...
	const yearMinLength = (() => {
		const yearMatch = format === null || format === void 0 ? void 0 : format.match(/y+/);
		if (!yearMatch || yearMatch[0].length <= 1) return null;
	...
	function onKeyDown(event) {
		...
		switch (event.key) {
			...
			case divider: {
				...
				focus(findInput(input, event.key === "ArrowLeft" ? "previousElementSibling" : "nextElementSibling"));
				break;
			}
			default:
		}
	...
	function onKeyUp(event) {
		const { key, target: input } = event;
		if (!(lastPressedKey.current === key)) return;
		...
		*/
		if (Number(value) * 10 > Number(max) || value.length >= max.length) focus(findInput(input, "nextElementSibling"));
	}
	/**
	...
	*/
	function onChangeExternal() {
		...
		const values = {};
		for (const formElement of formElements) values[formElement.name] = "valueAsNumber" in formElement ? formElement.valueAsNumber : Number(formElement.value);
		if (formElements.every((formElement) => !formElement.value)) {
			onChangeProps(null, false);
			return;
		}
		if (yearMinLength !== null && ((_a = yearInput.current) === null || _a === void 0 ? void 0 : _a.value) && yearInput.current.value.length < yearMinLength) return;
		...
		if (isEveryValueFilled && isEveryValueValid) {
			const year = Number(values.year || (/* @__PURE__ */ new Date()).getFullYear());
			...
			onChangeProps(getProcessedValue(proposedValue), false);
			return;
		}
	...
	*/
	...
	(0, import_react.useEffect)(() => {
		if (!hasPendingInternalChange.current) return;
	...
	*/
	...
	function renderDay(currentMatch, index) {
		...
		return (0, import_jsx_runtime.jsx)(DayInput, {
			...commonInputProps,
	...
	function renderMonth(currentMatch, index) {
		if (currentMatch && currentMatch.length > 4) throw new Error(`Unsupported token: ${currentMatch}`);
		if (currentMatch.length > 2) return (0, import_jsx_runtime.jsx)(MonthSelect, {
			...commonInputProps,
		...
		const showLeadingZerosFromFormat = currentMatch && currentMatch.length === 2;
		return (0, import_jsx_runtime.jsx)(MonthInput, {
			...commonInputProps,
			ariaLabel: monthAriaLabel,
			autoFocus: index === 0 && autoFocus,
			inputRef: monthInput,
	...
	function renderYear(_currentMatch, index) {
		return (0, import_jsx_runtime.jsx)(YearInput, {
			...commonInputProps,
	...
	function renderNativeInput() {
		return (0, import_jsx_runtime.jsx)(NativeInput, {
...
//#endregion
//#region node_modules/react-date-picker/dist/DatePicker.js
...
var CalendarIcon = (0, import_jsx_runtime.jsxs)("svg", {
	...iconProps,
	"aria-hidden": "true",
...
var ClearIcon = (0, import_jsx_runtime.jsxs)("svg", {
	...iconProps,
	"aria-hidden": "true",
	className: `${baseClassName}__clear-button__icon ${baseClassName}__button__icon`,
	children: [(0, import_jsx_runtime.jsx)("line", {
		x1: "4",
		x2: "15",
		y1: "4",
		y2: "15"
...
function DatePicker(props) {
	...
	const [isOpen, setIsOpen] = (0, import_react.useState)(isOpenProps);
	const wrapper = (0, import_react.useRef)(null);
	const calendarWrapper = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		setIsOpen(isOpenProps);
	}, [isOpenProps]);
	function openCalendar({ reason }) {
		if (shouldOpenCalendar) {
			if (!shouldOpenCalendar({ reason })) return;
		}
		setIsOpen(true);
		if (onCalendarOpen) onCalendarOpen();
	}
	const closeCalendar = (0, import_react.useCallback)(({ reason }) => {
		if (shouldCloseCalendar) {
			if (!shouldCloseCalendar({ reason })) return;
		}
		setIsOpen(false);
		if (onCalendarClose) onCalendarClose();
	}, [onCalendarClose, shouldCloseCalendar]);
	function toggleCalendar() {
		if (isOpen) closeCalendar({ reason: "buttonClick" });
		else openCalendar({ reason: "buttonClick" });
	}
	function onChange(value, shouldCloseCalendar = shouldCloseCalendarOnSelect) {
		if (shouldCloseCalendar) closeCalendar({ reason: "select" });
		if (onChangeProps) onChangeProps(value);
	}
	function onFocus(event) {
		if (onFocusProps) onFocusProps(event);
		if (disabled || isOpen || !openCalendarOnFocus || event.target.dataset.select === "true") return;
		openCalendar({ reason: "focus" });
	}
	const onKeyDown = (0, import_react.useCallback)((event) => {
		if (event.key === "Escape") closeCalendar({ reason: "escape" });
	}, [closeCalendar]);
	function clear() {
		onChange(null);
	}
	...
	const onOutsideAction = (0, import_react.useCallback)((event) => {
		const { current: wrapperEl } = wrapper;
		const { current: calendarWrapperEl } = calendarWrapper;
		const target = "composedPath" in event ? event.composedPath()[0] : event.target;
		if (target && wrapperEl && !wrapperEl.contains(target) && (!calendarWrapperEl || !calendarWrapperEl.contains(target))) closeCalendar({ reason: "outsideAction" });
	}, [closeCalendar]);
	const handleOutsideActionListeners = (0, import_react.useCallback)((shouldListen = isOpen) => {
		for (const event of outsideActionEvents) if (shouldListen) document.addEventListener(event, onOutsideAction);
	...
	function renderInputs() {
		...
		return (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				(0, import_jsx_runtime.jsx)(DateInput, {
					...ariaLabelProps,
					...placeholderProps,
					autoFocus,
				...
				clearIcon !== null && (0, import_jsx_runtime.jsx)("button", {
					...
					onFocus: stopPropagation,
					type: "button",
					children: typeof clearIcon === "function" ? (0, import_react.createElement)(clearIcon) : clearIcon
				}),
				calendarIcon !== null && !disableCalendar && (0, import_jsx_runtime.jsx)("button", {
					...
					onFocus: stopPropagation,
					type: "button",
					children: typeof calendarIcon === "function" ? (0, import_react.createElement)(calendarIcon) : calendarIcon
				})
			]
		});
	}
	function renderCalendar() {
		if (isOpen === null || disableCalendar) return null;
		const { calendarProps, portalContainer, value } = props;
		const className = `${baseClassName}__calendar`;
		const classNames = clsx(className, `${className}--${isOpen ? "open" : "closed"}`);
		const calendar = (0, import_jsx_runtime.jsx)(dist_default$2, {
			locale,
			maxDate,
			maxDetail,
			minDate,
		...
		return portalContainer ? (0, import_react_dom.createPortal)((0, import_jsx_runtime.jsx)("div", {
			ref: calendarWrapper,
			className: classNames,
			children: calendar
		}), portalContainer) : (0, import_jsx_runtime.jsx)(dist_default$1, { children: (0, import_jsx_runtime.jsx)("div", {
			ref: (ref) => {
				if (ref && !isOpen) ref.removeAttribute("style");
			},
			className: classNames,
			children: calendar
		}) });
	}
	const eventProps = (0, import_react.useMemo)(() => makeEventProps(otherProps), [otherProps]);
	return (0, import_jsx_runtime.jsxs)("div", {
		className: clsx(baseClassName, `${baseClassName}--${isOpen ? "open" : "closed"}`, `${baseClassName}--${disabled ? "disabled" : "enabled"}`, className),
		"data-testid": dataTestid,
		id,
		...eventProps,
		onFocus,
...
//#endregion
//#region node_modules/react-date-picker/dist/index.js
var dist_default = DatePicker;
//#endregion
export { DatePicker, dist_default as default };
